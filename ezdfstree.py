...
skiplist = { 
                   '/proc': [ 'linux-armv7l', 'linux' ],
            '/sys/devices': [ 'linux-armv7l', 'linux' ],
                    '/dev': [ 'linux-armv7l', 'linux' ],
                '/sys/dev': [ 'linux-armv7l', 'linux' ],
	'///C:/%24recycle.bin': [ 'win32' ]
}

alternateLogDirHash = {
        'linux':os.getenv( 'EXTERNAL_STORAGE' ),
 'linux-armv7l':os.getenv( 'EXTERNAL_STORAGE' ),
		'win32':os.getenv( 'TEMP' ),
}

logDirHash = { 
        'linux': '/storage/extSdCard/00/log/tox',
 'linux-armv7l': '/mnt/sdcard/00/log/tox',
		'win32': 'C:/etc/tox'
}

drpBxDirHash = { 
        'linux': '/storage/emulated/0/00/log/tox',
...
class InputMergeObj:  # {{{
	def __init__( self, fName ):
		...
		try:
			...
			#
			self.H = open( fName, 'rt', encoding=fsEncoding )
...
def coerse2str( dataIn ): # {{{
	...
	# In this case, Python uses the surrogateescape encoding error handler, which means that undecodable 
	...
	if isinstance( dataIn, bytes ):
		retVal = dataIn.decode( fsEncoding, errors='surrogateescape' ) 
	elif isinstance( dataIn, str ):
		dataInBytes = dataIn.encode( fsEncoding, errors='surrogateescape') 
		retVal = dataInBytes.decode( fsEncoding, errors='surrogateescape'  )
	else:
		errMsg  = "** <coerse2str> dataIn == "
		errmsg += dataIn
		errmsg += " 'dataIn' must be bytes or str **"
		sys.exit( errMsg )
...
while len( dirStack ): # {{{ Main Outer Loop
	...
	urlPath = urllib.request.pathname2url( coerse2str( thisDir ) )
	if urlPath in skiplist : 
		osList = skiplist[ urlPath ]
		if sys.platform in osList :
			continue
	try:
